...
class NougatService {
    static Future<({int exitCode, int resultCode, String? content, String? fileName, StringBuffer? stdoutBuf})> convertPdf(String nougatExe, String outputDir, selectedPdf, Function(ConverProgress data) onProgress) async{
      ...
      process.stderr.transform(SystemEncoding().decoder).listen((chunk) {
        ...
        if (match != null) {
          ...
          onProgress(ConverProgress(percent: percent, current: current, total: total));
        }
      });
    ...
    static Future<({int resultCode, String? contentData, String? fileName })> scanFileMd(String outputDir) async{
      ...
      return (resultCode: 1, contentData: content, fileName: files.first.path.split(r"\\" ).last);
    }
}